...
export const getReviews = ( args ) => {
	return apiFetch( {
		path:
			'/jankx/store/v1/products/reviews?' +
			Object.entries( args )
				.map( ( arg ) => arg.join( '=' ) )
				.join( '&' ),
		parse: false,
...
export const getBlockClassName = ( attributes ) => {
	...
	let blockClassName = 'jankx-block-all-reviews';

	if ( productId ) {
		blockClassName = 'jankx-block-reviews-by-product';
	}

	if ( Array.isArray( categoryIds ) ) {
		blockClassName = 'jankx-block-reviews-by-category';
	}
